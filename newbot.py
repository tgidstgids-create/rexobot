...
# Expiry and Group Settings
DEFAULT_EXPIRY_HOURS = 24  # Default 24 hours expiry
GROUP_JOIN_REQUIRED = True  # Whether group join is mandatory
...
try:
    ...
    groups_col = db['groups']  # Store group links
    user_groups_col = db['user_groups']  # Track user group joins
    expiry_settings_col = db['expiry_settings']  # Store expiry duration settings
    
    # Create indexes
    coupons_col.create_index([("coupon_code", 1)], unique=True)
    coupons_col.create_index([("status", 1)])
    coupons_col.create_index([("created_at", -1)])
    groups_col.create_index([("country", 1)], unique=True)
    user_groups_col.create_index([("user_id", 1), ("group_id", 1)], unique=True)
    expiry_settings_col.create_index([("country", 1)], unique=True)
    
    logger.info("✅ MongoDB connected successfully with all indexes")
...
# -----------------------
def set_country_expiry(country, hours):
    """Set expiry duration for a country's accounts"""
    try:
        if hours < 1:
            return False, "Hours must be at least 1"
        
        expiry_settings_col.update_one(
            {"country": country},
            {"$set": {
                "country": country,
                "expiry_hours": hours,
        ...
        return True, f"Expiry set to {hours} hours for {country}"
    except Exception as e:
        logger.error(f"Error setting expiry: {e}")
        return False, str(e)

def get_country_expiry(country):
    """Get expiry duration for a country"""
    setting = expiry_settings_col.find_one({"country": country})
    if setting:
        return setting.get("expiry_hours", DEFAULT_EXPIRY_HOURS)
    return DEFAULT_EXPIRY_HOURS

def calculate_expiry_time(country):
    """Calculate expiry time for new purchase"""
    hours = get_country_expiry(country)
    return datetime.utcnow() + timedelta(hours=hours)

def check_expired_orders():
    """Background task to check and mark expired orders"""
    while True:
        try:
            # Find orders that have expired but not marked as expired
            expired_orders = orders_col.find({
            ...
            for order in expired_orders:
                ...
                try:
                    bot.send_message(
                        user_id,
                        f"⚠️ **Account Expired**\n\n"
                        f"Your purchased account has expired after {get_country_expiry(order.get('country'))} hours.\n"
                        f"Buy a new account to continue using the service.\n\n"
                        f"🛒 Click below to buy again:",
                ...
                logger.info(f"Order {order_id} expired for user {user_id}")
...
expiry_thread = threading.Thread(target=check_expired_orders, daemon=True)
expiry_thread.start()

# -----------------------
# GROUP JOIN SYSTEM FUNCTIONS
# -----------------------
def set_country_groups(country, group_links):
    """Set required groups for a country"""
    try:
        groups_col.update_one(
            {"country": country},
            {"$set": {
                "country": country,
                "group_links": group_links,  # List of @usernames or invite links
                "updated_at": datetime.utcnow()
            }},
            upsert=True
        )
        return True, f"Groups set for {country}"
    except Exception as e:
        logger.error(f"Error setting groups: {e}")
        return False, str(e)

def get_country_groups(country):
    """Get required groups for a country"""
    groups = groups_col.find_one({"country": country})
    if groups:
        return groups.get("group_links", [])
    return []

def has_user_joined_groups(user_id, country):
    """Check if user has joined all required groups for a country"""
    try:
        ...
        user_groups = user_groups_col.find_one({"user_id": user_id})
        if not user_groups:
            return False, groups
        
        joined = user_groups.get("joined_groups", [])
        missing = [g for g in groups if g not in joined]
        
        return len(missing) == 0, missing
        
    except Exception as e:
        logger.error(f"Error checking group joins: {e}")
        return False, []

def mark_group_joined(user_id, group_link):
    """Mark that user has joined a specific group"""
    try:
        user_groups_col.update_one(
            {"user_id": user_id},
            {
                "$addToSet": {"joined_groups": group_link},
        ...
        logger.error(f"Error marking group join: {e}")
        return False
...
Click the button below to join, then press VERIFY ✅</blockquote>"""
        ...
        # Redeem Coupon callback
            ...
            # Check if group join is required
            if GROUP_JOIN_REQUIRED:
                joined_all, missing = has_user_joined_groups(user_id, country_name)
                
                if not joined_all and missing:
                    # Show group join verification page
                    groups_text = "\n".join([f"• {g}" for g in missing])
                    
                    markup = InlineKeyboardMarkup(row_width=1)
                    for group in missing:
                        # Add join button for each missing group
                        if group.startswith("https://"):
                            markup.add(InlineKeyboardButton(f"📢 Join Group", url=group))
                        else:
                            markup.add(InlineKeyboardButton(f"📢 Join {group}", url=f"https://t.me/{group.replace('@', '')}"))
                    
                    markup.add(InlineKeyboardButton("✅ Verified - Check Again", callback_data="verify_group_join"))
                    markup.add(InlineKeyboardButton("⬅️ Back to Countries", callback_data="back_to_countries"))
                    
                    # Store country in state
                    group_verify_state[user_id] = {"country": country_name}
                    
                    edit_or_resend(
                        call.message.chat.id,
                        call.message.message_id,
                        f"<blockquote><b>📢 Join Required Groups</b></blockquote>\n\n"
                        f"To purchase <b>{country_name}</b> accounts, you must join these groups:\n\n"
                        f"{groups_text}\n\n"
                        f"Click each button to join, then press VERIFY ✅",
                        markup=markup,
                        parse_mode="HTML"
                    )
                    return
            
            # If no groups required or already joined, show country details
            show_country_details(user_id, country_name, call.message.chat.id, call.message.message_id, call.id)
        
        elif data == "verify_group_join":
            user_id = call.from_user.id
            chat_id = call.message.chat.id
            message_id = call.message.message_id
            
            # Get country from stored state
            country = group_verify_state.get(user_id, {}).get("country")
            if not country:
                bot.answer_callback_query(call.id, "❌ Session expired", show_alert=True)
                return
            
            # Check if user joined all groups
            joined_all, missing = has_user_joined_groups(user_id, country)
            
            if joined_all:
                # ✅ User joined all groups, proceed to purchase
                group_verify_state.pop(user_id, None)
                
                # Show country details
                show_country_details(user_id, country, chat_id, message_id, call.id)
                bot.answer_callback_query(call.id, "✅ Verification successful! Proceed with purchase.", show_alert=True)
            else:
                # ❌ Still missing groups
                missing_text = "\n".join([f"• {g}" for g in missing])
                bot.answer_callback_query(
                    call.id, 
                    f"❌ Please join all groups first!\nMissing:\n{missing_text}", 
                    show_alert=True
                )
        ...
        # Recharge methods (UPI and Crypto)
        ...
        # Admin Coupon Management callbacks
        ...
        # EXPIRY MANAGEMENT CALLBACKS
        elif data == "manage_expiry":
            if is_admin(user_id):
                bot.answer_callback_query(call.id, "⏰ Managing expiry settings...")
                show_expiry_management(call.message.chat.id, call.message.message_id)
            else:
                bot.answer_callback_query(call.id, "❌ Unauthorized", show_alert=True)

        elif data.startswith("set_expiry_"):
            if is_admin(user_id):
                country_name = data.replace("set_expiry_", "")
                expiry_state[user_id] = {"country": country_name, "step": "waiting_hours"}
                edit_or_resend(
                    call.message.chat.id,
                    call.message.message_id,
                    f"🌍 Country: {country_name}\n"
                    f"⏰ Current Expiry: {get_country_expiry(country_name)} hours\n\n"
                    f"Enter new expiry hours (minimum 1):",
                    markup=InlineKeyboardMarkup().add(
                        InlineKeyboardButton("❌ Cancel", callback_data="manage_expiry")
                    )
                )
            else:
                bot.answer_callback_query(call.id, "❌ Unauthorized", show_alert=True)
        ...
        # GROUP MANAGEMENT CALLBACKS
            ...
            if is_admin(user_id):
                bot.answer_callback_query(call.id, "👥 Managing group requirements...")
                show_group_management(call.message.chat.id, call.message.message_id)
            else:
                bot.answer_callback_query(call.id, "❌ Unauthorized", show_alert=True)

        elif data.startswith("set_groups_"):
            if is_admin(user_id):
                country_name = data.replace("set_groups_", "")
                group_state[user_id] = {"country": country_name, "step": "waiting_groups"}
                
                current_groups = get_country_groups(country_name)
                current_text = "\n".join([f"• {g}" for g in current_groups]) if current_groups else "None"
                
                edit_or_resend(
                    call.message.chat.id,
                    call.message.message_id,
                    f"🌍 Country: {country_name}\n"
                    f"👥 Current Required Groups:\n{current_text}\n\n"
                    f"Enter group links (one per line):\n"
                    f"Example:\n@group1\n@group2\nhttps://t.me/group3",
                    markup=InlineKeyboardMarkup().add(
...
def show_country_details(user_id, country_name, chat_id, message_id, callback_id):
    ...
    try:
        ...
        # Get expiry hours for this country
        expiry_hours = get_country_expiry(country_name)
...
def handle_logout_session(user_id, session_id, chat_id, callback_id):
    ...
    try:
        ...
        if success:
            ...
            # Add Main Menu button to logout success message
            markup = InlineKeyboardMarkup()
...
# -----------------------
# ADMIN UI: EXPIRY MANAGEMENT
# -----------------------
def show_expiry_management(chat_id, message_id=None):
    """Show expiry management menu"""
    if not is_admin(chat_id):
        return
    
    countries = get_all_countries()
    text = "⏰ **Expiry Management**\n\nSet how long accounts remain active after purchase.\n\n"
    
    markup = InlineKeyboardMarkup(row_width=2)
    
    for country in countries:
        expiry = get_country_expiry(country['name'])
        text += f"• {country['name']}: {expiry} hours\n"
        markup.add(InlineKeyboardButton(
            f"✏️ {country['name']} ({expiry}h)",
            callback_data=f"set_expiry_{country['name']}"
        ))
    
    markup.add(InlineKeyboardButton("⬅️ Back to Admin", callback_data="admin_panel"))
    
    if message_id:
        edit_or_resend(chat_id, message_id, text, markup=markup, parse_mode="Markdown")
    else:
        bot.send_message(chat_id, text, reply_markup=markup, parse_mode="Markdown")
...
# -----------------------
def show_group_management(chat_id, message_id=None):
    """Show group management menu"""
    if not is_admin(chat_id):
        return
    
    countries = get_all_countries()
    text = "👥 **Group Management**\n\nSet required groups users must join before purchasing.\n\n"
    
    markup = InlineKeyboardMarkup(row_width=2)
    
    for country in countries:
        groups = get_country_groups(country['name'])
        group_count = len(groups)
        text += f"• {country['name']}: {group_count} groups required\n"
        markup.add(InlineKeyboardButton(
            f"✏️ {country['name']} ({group_count} groups)",
            callback_data=f"set_groups_{country['name']}"
        ))
...
# UPI Payment Flow Message Handlers
...
# -----------------------
# EXPIRY HOURS MESSAGE HANDLER
# -----------------------
@bot.message_handler(func=lambda m: expiry_state.get(m.from_user.id, {}).get("step") == "waiting_hours")
def handle_expiry_hours_input(msg):
    user_id = msg.from_user.id
    
    if user_id not in expiry_state or expiry_state[user_id]["step"] != "waiting_hours":
        return
    
    if not is_admin(user_id):
        bot.send_message(msg.chat.id, "❌ Unauthorized access")
        expiry_state.pop(user_id, None)
        return
    
    try:
        hours = float(msg.text.strip())
        if hours < 1:
            bot.send_message(msg.chat.id, "❌ Hours must be at least 1. Enter again:")
            return
        
        country = expiry_state[user_id]["country"]
        success, message = set_country_expiry(country, hours)
        ...
        if success:
            bot.send_message(
                ...
                f"✅ **Expiry Updated!**\n\n"
                f"🌍 Country: {country}\n"
                f"⏰ New Expiry: {hours} hours\n\n"
                f"All new purchases for {country} will expire after {hours} hours."
            )
        else:
            bot.send_message(msg.chat.id, f"❌ Error: {message}")
        
        expiry_state.pop(user_id, None)
        show_expiry_management(msg.chat.id)
        
    except ValueError:
        bot.send_message(msg.chat.id, "❌ Invalid number. Enter hours (e.g., 24):")

# -----------------------
# GROUP LINKS MESSAGE HANDLER
# -----------------------
@bot.message_handler(func=lambda m: group_state.get(m.from_user.id, {}).get("step") == "waiting_groups")
def handle_group_links_input(msg):
    user_id = msg.from_user.id
    
    if user_id not in group_state or group_state[user_id]["step"] != "waiting_groups":
        return
    
    if not is_admin(user_id):
        bot.send_message(msg.chat.id, "❌ Unauthorized access")
        group_state.pop(user_id, None)
        return
    
    # Parse group links (one per line)
    text = msg.text.strip()
    group_links = [line.strip() for line in text.split('\n') if line.strip()]
    
    # Basic validation
    valid_links = []
    for link in group_links:
        if link.startswith('@') or link.startswith('https://t.me/') or link.startswith('t.me/'):
            valid_links.append(link)
    
    if not valid_links:
        bot.send_message(
            msg.chat.id,
            "❌ No valid group links found.\n"
            "Use formats: @username, https://t.me/username, t.me/username\n\n"
            "Enter group links again:"
        )
        return
    
    country = group_state[user_id]["country"]
    success, message = set_country_groups(country, valid_links)
    
    if success:
        groups_text = "\n".join([f"• {g}" for g in valid_links])
        bot.send_message(
            msg.chat.id,
            f"✅ **Groups Updated!**\n\n"
            f"🌍 Country: {country}\n"
            f"👥 Required Groups:\n{groups_text}\n\n"
            f"Users must join these groups before purchasing {country} accounts."
        )
    else:
        bot.send_message(msg.chat.id, f"❌ Error: {message}")
    
    group_state.pop(user_id, None)
    show_group_management(msg.chat.id)
...
def show_admin_panel(chat_id):
    ...
    markup.add(
        ...
        InlineKeyboardButton("⏰ Expiry Settings", callback_data="manage_expiry")
    )
    markup.add(
        InlineKeyboardButton("👥 Group Settings", callback_data="manage_groups"),
...
# -----------------------
# PROCESS PURCHASE FUNCTION (UPDATED WITH EXPIRY)
# -----------------------
def process_purchase(user_id, account_id, chat_id, message_id, callback_id):
    try:
        ...
        expiry_time = calculate_expiry_time(account['country'])
        expiry_hours = get_country_expiry(account['country'])
        ...
        time_remaining = (expiry_time - datetime.utcnow()).total_seconds() / 3600
...
📱 Phone Number: `{account.get('phone', 'N/A')}`"""
        ...
        account_details += f"⏳ Account will expire in {expiry_hours} hours"
        ...
        bot.answer_callback_query(callback_id, f"✅ Purchase successful! Account expires in {expiry_hours} hours.", show_alert=True)
...
if __name__ == "__main__":
    ...
    logger.info(f"Expiry System: Enabled with default {DEFAULT_EXPIRY_HOURS} hours")
    logger.info(f"Group Join System: {'Enabled' if GROUP_JOIN_REQUIRED else 'Disabled'}")
