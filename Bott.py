...
# MUST JOIN CHANNELS - NOW TWO CHANNELS
MUST_JOIN_CHANNEL_1 = "@CUTE_OTP_HUB"
MUST_JOIN_CHANNEL_2 = "@CUTE_OTP_LOGS"  # Add your second channel here
...
# -----------------------
# UPDATED: CHANNEL VERIFICATION FUNCTIONS (DUAL CHANNEL)
# -----------------------
def has_user_joined_channels(user_id):
    ...
    try:
        ...
        return channel1_joined and channel2_joined
    except Exception as e:
        logger.error(f"Error checking channel membership: {e}")
        return False

def get_channels_not_joined(user_id):
    ...
    try:
        ...
        try:
            ...
            if member1.status not in ['member', 'administrator', 'creator']:
                not_joined.append(MUST_JOIN_CHANNEL_1)
        except:
            not_joined.append(MUST_JOIN_CHANNEL_1)
        ...
        try:
            ...
            if member2.status not in ['member', 'administrator', 'creator']:
                not_joined.append(MUST_JOIN_CHANNEL_2)
        except:
            not_joined.append(MUST_JOIN_CHANNEL_2)
        
        return not_joined
    except Exception as e:
        logger.error(f"Error getting channels not joined: {e}")
...
def start(msg):
    ...
    if not has_user_joined_channels(user_id):
        not_joined = get_channels_not_joined(user_id)
        
        # Show join message with both channels
        channel_links = ""
        for channel in not_joined:
            channel_links += f"👉 <a href='https://t.me/{channel[1:]}'>{channel}</a>\n"
        
        caption = f"""<blockquote><b>🚀 Join Our Channels First!</b>

📢 To use this bot, you must join BOTH official channels.

{channel_links}
👉 Get updates, new features & support from our channels.

Click the buttons below to join, then press VERIFY ✅</blockquote>"""
        
        # Create markup with join buttons for each channel not joined
        markup = InlineKeyboardMarkup(row_width=2)
    ...
    try:
        ...
        if data == "verify_join":
            if has_user_joined_channels(user_id):
                ...
                logger.info(f"User verified join for both channels: {user_id}")
                ...
                # ❌ User not joined both channels → show updated message
                not_joined = get_channels_not_joined(user_id)
...
# -----------------------
# UPDATED: COUNTRY SELECTION FUNCTIONS WITH GRID LAYOUT
# -----------------------
def show_countries(chat_id):
    ...
    # Format message with country list and stock info (like in the image)
    text = "🇮🇳 **OTP Bazar Bot**\n\n"
    
    # Show each country with stock and price
    for country in countries:
        stock = get_available_accounts_count(country['name'])
        price = format_currency(country['price'])
        text += f"**{country['name']}**\n"
        text += f"📦 Stock: {stock} | 💰 Price: {price}\n\n"
    
    text += "Tap a country below to Buy:"
    
    # Create markup with 3 columns grid
    markup = InlineKeyboardMarkup(row_width=3)
    
    # Add country buttons in a grid (3 per row)
    country_buttons = []
    for country in countries:
    ...
    # Add buttons in batches of 3
    for i in range(0, len(country_buttons), 3):
        markup.add(*country_buttons[i:i+3])
...
def show_country_details(user_id, country_name, chat_id, message_id, callback_id):
    ...
    try:
        ...
        text = f"🇮🇳 **OTP Bazar Bot**\n\n"
        text += f"**{country_name}**\n"
        text += f"📦 Stock: {accounts_count} | 💰 Price: {format_currency(country['price'])}\n\n"
        text += "🔍 Reliable | Affordable | Good Quality\n\n"
        text += "⚠️ Use Telegram X only to login.\n"
        text += "🚫 Not responsible for freeze / ban."
